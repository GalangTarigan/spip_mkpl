...
function showNotifications(notifications, target, incomingNotif) {
    if(notifications.length) {
        ...
        $(target).parent().append('<div class="text-center pt50 no-msg"><span>Tidak ada pesan yang belum terbaca</span></div>');
    }
}
...
function routeNotification(notification) {
    ...
    if(notification.type === NOTIFICATION_TYPES.newReport) {
        ...
        to = `admin/daftar-proyek-instalasi/detail-proyek?laporan=${id_laporan}` + to;
    }else if(notification.type === NOTIFICATION_TYPES.newTReport){
        var id_laporan = notification.data.id_laporan
        to = `admin/daftar-proyek-instalasi/detail-proyek?laporan=${id_laporan}` + to;
    }else if(notification.type === NOTIFICATION_TYPES.newCReport){
        var id_laporan = notification.data.id_laporan
        to = `admin/keluhan/detail-keluhan?keluhan=${id_laporan}`+ to;
    }
    return '/' + to;
}
...
function makeNotificationText(notification) {
    var text = '', nama, instansi
    if(notification.type === NOTIFICATION_TYPES.newReport) {
        nama = notification.data.user_name
        instansi = notification.data.nama_instansi
        text+= '<div class="notification-icon"><i class="fas fa-file-signature"></i></div><div class="notification-content">'+nama+' membuat laporan instalasi '+'<strong>'+instansi+'</strong></div><div class="row"></div><div class="notification-time">'+diffDates(notification.created_at)+'</div></div>'
    }else if(notification.type === NOTIFICATION_TYPES.newTReport){
        nama = notification.data.user_name
        instansi = notification.data.nama_instansi
        text += '<div class="notification-icon"><i class="fas fa-chalkboard-teacher"></i></div><div class="notification-content">Laporan training <strong>'+instansi+'</strong> berhasil dibuat oleh '+nama+'</div><div class="notification-time">'+diffDates(notification.created_at)+'</div>'
    }else if(notification.type === NOTIFICATION_TYPES.newCReport){
        nama = notification.data.user_name
        instansi = notification.data.nama_instansi
        text += '<div class="notification-icon"><i class="fab fa-teamspeak"></i></div><div class="notification-content">'+nama+' membuat laporan keluhan '+'<strong>'+instansi+'</strong></div><div class="notification-time">'+diffDates(notification.created_at)+'</div>'
    }
    return text;
};
